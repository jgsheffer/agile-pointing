...
// Add this to the existing session handling
function getOrCreateSession(sessionId, room, name, avatar) {
  if (!sessionId || !sessions.has(sessionId)) {
    sessionId = uuidv4();
  }

  // Preserve existing room data if session exists
  if (sessions.has(sessionId)) {
    const existingSession = sessions.get(sessionId);
    room = room || existingSession.room;
    name = name || existingSession.name;
    avatar = avatar || existingSession.avatar;
  }

  sessions.set(sessionId, { room, name, avatar });
  return sessionId;
}
