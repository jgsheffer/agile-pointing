...
function logRoomState(roomId, action, roomType = 'retro') {
    ...
    console.log(`[${action}] ${roomType} room ${roomId}:`, {
        participants: room ? (roomType === 'retro' ? Array.from(room.participants) : Array.from(room.values()).map(p => p.name)) : [],
...
io.on('connection', (socket) => {
    ...
    socket.on('joinRoom', ({ room, name, avatar, sessionId }) => {
        ...
        io.to(room).emit('updateParticipants', Array.from(pointingRooms.get(room).values()));
    ...
    socket.on('vote', ({ room, vote, sessionId }) => {
        if (pointingRooms.has(room) && pointingRooms.get(room).has(sessionId)) {
            ...
            io.to(room).emit('updateVotes', Array.from(pointingRooms.get(room).values()));
        }
    });

    socket.on('revealVotes', (room) => {
        io.to(room).emit('votesRevealed', Array.from(pointingRooms.get(room).values()));
    });
    ...
    socket.on('resetVotes', (room) => {
        if (pointingRooms.has(room)) {
            ...
            io.to(room).emit('votesReset', Array.from(pointingRooms.get(room).values()));
        }
    });
...
setInterval(() => {
    ...
    retroRooms.forEach((room, roomId) => {
        if (room.participants.size === 0 && (now - room.lastActivity) > INACTIVE_THRESHOLD) {
